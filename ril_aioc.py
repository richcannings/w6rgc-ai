...
class RadioInterfaceLayerAIOC:
    ...
    def _find_aioc_device(self):
        """
        Finds the AIOC audio device by name and sets its properties.
        """
        devices = sd.query_devices()
        for i, device in enumerate(devices):
            if 'AIOC' in device['name']:
        ...
        raise RuntimeError("AIOC device not found. Please check your USB connection.")
        ...
        for attempt in range(max_retries):
            if not self.check_carrier_sense():
                try:
                    ...
                    self.serial_conn.setDTR(True) # AIOC may use DTR
                    time.sleep(0.1)
    ...
    def play_audio(self, audio_data, sample_rate):
        ...
        try:
            print(f"🔊 Playing audio via AIOC on device {self.audio_device_index} at {self.samplerate} Hz...")
            sd.play(audio_data, samplerate=self.samplerate, device=self.audio_device_index, blocking=True)
        except Exception as e:
            print(f"Error during AIOC audio playback: {e}")
    ...
    def close(self):
        ...
        if self.serial_conn and self.serial_conn.is_open:
            ...
            print("AIOC serial connection closed.")
